...
class DB():
    ...
    def get_config(self):
        ...
        self.ephem_config = {   "lat": dict["lat"],
                                "lon": dict["lon"],
                                "elev": dict["elev"]}
    ...
    def get_summary_graph(self, cumsum_date, date=None, days=7):
        ...
        dict = {}
        for d in dates:                                                 # 各日付において
            dict[d] = { "lighting_minutes_sum": df.at[d, "lighting_minutes_sum"],
                        "mean_temp_sum": df.at[d, "mean_temp_sum"],
                        }                                               # 日ごとの辞書として登録する
        ...
        # 温度のグラフ
        fig, ax = self.getEmptyFig()
        y_sum = [item["mean_temp_sum"] for item in dict.values()]
        ax.plot(x, y_sum, ".-b", linewidth=2)
        ax.set_title(f"{cumsum_date}からの平均気温の累計（度）")
        ax.set_ylim(bottom=0)
        for i, value in enumerate(y_sum):
            ax.text(x[i], y_sum[i]+ 2, round(value, 1))
            fig.canvas.draw()
        ...
        # 点灯時間のグラフ
        fig, ax = self.getEmptyFig()
        y = [item["lighting_minutes_sum"] for item in dict.values()]
        ax.plot(x, y, ".-", linewidth=2, color="orange")
        ax.set_title(f"{cumsum_date}からのLED点灯時間の累計（分）")
        ax.set_ylim(bottom=0)
...
def main():
    db.get_summary_graph("2024/03/01")
    print("done.")
