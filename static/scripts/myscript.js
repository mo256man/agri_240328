...
let tab = "main";                   // 起動時に表示するタブ
let lights = "○−○−○";               // 光センサーの戻り値
let temp, humi                      // 温度と湿度
...
async function do1st() {
    ...
    const kbOpt = {showAnim: "slideDown", showOptions: null, duration:"fast", showOn:"button",
                   layout:['789'+$.keypad.CLOSE, '456'+$.keypad.CLEAR, '123'+$.keypad.BACK, '-0']};    // 数値キーパッドのオプション
    const ids = ["lat", "lon", "elev", "morning_offset", "evening_offset", "morning_minutes", "evening_minutes",
                "sensing_interval", "sensing_count", "year", "month", "day", "hour", "minute",
    ...
    await $.ajax("/getContec", {
        ...
        try {                                                           // センサー値取得できていたら
            ...
            if (volt_status == 3 ) {                                    // 電圧状態3ならば
                $(".batt_3").css("visibility","visible");               // グラフの青バーを表示
                $(".batt_2").css("visibility","visible");               // グラフの緑バーを表示
            } else if (volt_status == 2) {                              // 電圧状態2ならば
                $(".batt_3").css("visibility","hidden");                // グラフの青バーを非表示
                $(".batt_2").css("visibility","visible");               // グラフの緑バーを表示
            } else {                                                    // いずれでもなければ　つまり電圧状態1ならば
                $(".batt_3").css("visibility","hidden");                // グラフの青バーを非表示
                $(".batt_2").css("visibility","hidden");                // グラフの緑バーを非表示
            ...
            if (isLightCnt) {                                           // 光センサー積算する設定ならば
                ...
                let lightlog = dict["log"].slice(0,5);                  // コンテック出力（光センサー＋電圧リレー計8文字）から光センサーを切り出す
                lightlog = lightlog.replace(/○/g, "☁");
                lightlog = lightlog.replace(/−/g, "☼");
                addLightLog(time + "　#" + (dict["light_cnt"]) + "　" + lightlog);
                addMsg(time + "　天候：" + lightlog);
                ...
                if (dict["light_cnt"] == sensing_count) {               // 指定した回数だけセンサー値を測定したら
                    ...
                    const condWeather = (dict["light_sum"]>=th ? "曇":"晴");   // 三項演算子　しきい値より以上かどうか
                    const lastStatus = (lastIsLED ? "点灯":"消灯");             // 三項演算子　さっきまで点灯していたかどうか
                    const composedCond = `${condWeather},${lastStatus},${volt_status}`;     // テンプレートリテラル　3条件を並べる
                    
                    let beginON = false;                                // 消灯状態から点灯状態になったときtrueになるブール値
                    let beginOFF = false;                               // 点灯状態から消灯状態になったときtrueになるブール値
                    
                    switch (composedCond) {
                        case "曇,点灯,1":                            // 曇りで、それまで点灯していて、バッテリー残量少の場合
                            isLED = false                           // 点灯しない
                            msg = "バッテリーが不足気味なので消灯します";
                            comment = "バッテリー不足につき消灯開始";
                            beginOFF = true;
                            break;
                        case "曇,点灯,2":                            // 曇りで、それまで点灯していて、バッテリー残量普通の場合
                            isLED = true                            // 点灯する
                            msg = "点灯継続します";
                            comment = "点灯継続";
                            break;
                        case "曇,点灯,3":                            // 曇りで、それまで点灯していて、バッテリー残量十分の場合
                            isLED = true                            // 点灯する
                            msg = "バッテリー十分あるので点灯継続します";
                            comment = "バッテリー十分につき点灯継続";
                            break;
                        case "曇,消灯,1":                            // 曇りで、それまで消灯していて、バッテリー残量少の場合
                            isLED = false                           // 点灯しない
                            msg = "バッテリーが不足気味なので消灯します";
                            comment = "バッテリー不足につき消灯継続";
                            break;
                        case "曇,消灯,2":                            // 曇りで、それまで消灯していて、バッテリー残量普通の場合
                            isLED = true                            // 点灯する
                            msg = "曇っているので点灯します";
                            comment = "点灯開始";
                            beginOn = true;
                            break;
                        case "曇,消灯,3":                            // 曇りで、それまで消灯していて、バッテリー残量十分の場合
                            isLED = true                            // 点灯する
                            msg = "曇っているので点灯します";
                            comment = "点灯開始";
                            beginOn = true;
                            break;
                        case "晴,点灯,1":                            // 晴れで、それまで点灯していて、バッテリー残量少の場合
                            isLED = false                           // 点灯しない
                            msg = "晴れているので消灯します";
                            comment = "消灯開始";
                            beginOFF = true;
                            break;
                        case "晴,点灯,2":                            // 晴れで、それまで点灯していて、バッテリー残量普通の場合
                            isLED = false                           // 点灯しない
                            msg = "晴れているので消灯します";
                            comment = "消灯開始";
                            beginOFF = true;
                            break;
                        case "晴,点灯,3":                            // 晴れで、それまで点灯していて、バッテリー残量十分場合
                            isLED = true                            // 点灯する
                            msg = "バッテリー十分あるので点灯継続します";
                            comment = "バッテリー十分につき点灯継続";
                            break;
                        case "晴,消灯,1":                            // 晴れで、それまで消灯していて、バッテリー残量少の場合
                            isLED = false                           // 点灯しない
                            msg = "消灯継続します";
                            comment = "消灯継続";
                            break;
                        case "晴,消灯,2":                            // 晴れで、それまで消灯ていて、バッテリー残量普通の場合
                            isLED = false                           // 点灯しない
                            msg = "消灯継続します";
                            comment = "消灯継続";
                            break;
                        case "晴,消灯,3":                            // 晴れで、それまで消灯していて、バッテリー残量十分の場合
                            isLED = true                            // 点灯する
                            msg = "バッテリー十分あるので点灯します";
                            comment = "バッテリー十分につき点灯開始";
                            beginOn = true;
                            break;
                        default:
                            msg = "条件エラー"
                            consoleTimeLog("条件エラー");
                    }
                    
                    if (beginON) {                                      // 消灯から点灯に変わったのならば
                        lightOnTime = dayjs();                          // 点灯開始時刻を覚えておく　文字列ではなくdayjs形式で
                    }

                    if (beginOFF) {                                     // 点灯から消灯に変わったのならば
                        const lightSeconds = dayjs().diff(lightOnTime, "seconds");      // 点灯時間（秒）
                        const lightMinutes = Math.round(lightSeconds/60);               // 秒を分にする
                    }
